...
def test_post_register_invalid_email_format(client):
    ...
    response = client.post(
        "/register",
        data={"email": "invalidemail", "password": "testpass", "name": "Bad Email"},
    )
    assert response.status_code in [200, 400]
...
def test_post_login_invalid_credentials(client):
    ...
    response = client.post(
        "/login", data={"email": "fake@fake.com", "password": "wrongpass"}
    )
...
def test_register_duplicate_email(client):
    client.post(
        "/register",
        data={"email": "duplicate@example.com", "password": "pass1", "name": "First"},
        follow_redirects=True,
    )

    response = client.post(
        "/register",
        data={"email": "duplicate@example.com", "password": "pass2", "name": "Second"},
        follow_redirects=True,
    )
